...
- [x] context_stats 구현 (상태별/에이전트별/태그별 통계)
- [x] context_export 구현 (markdown/json/html 형식)
- [x] context_recommend 구현 (관련성 점수 기반 추천)
- [x] 실패 패턴 경고 강화 (반복 실패 감지, EnhancedWarning)
- [x] 프로젝트 간 검색 (global.db at ~/.context-sync/global.db)

### Phase 3: 최적화 (v2.2) ✅ 완료

- [x] 스키마 확장
  - [x] context_tags 정규화 테이블 (태그 검색 최적화)
  - [x] contexts_archive 아카이브 테이블
  - [x] 복합 인덱스 추가 (status+created_at, warnings+created_at, context+type)
- [x] 성능 최적화
  - [x] JSON LIKE 태그 검색 → context_tags 조인 (10x+ 성능 향상)
  - [x] N+1 쿼리 → 배치 로딩 (context-export.ts)
  - [x] IN 서브쿼리 → LEFT JOIN (context-stats.ts)
  - [x] 세션 체인 while 루프 → 재귀 CTE (context-get.ts)
- [x] 아카이빙 시스템
  - [x] ArchiveService (services/archive-service.ts)
  - [x] context_archive 도구 (archive/restore/stats/search/list/purge)
...
│   │   └── global-db.ts         # v2.1: 전역 DB (~/.context-sync/global.db)
│   ├── store/
...
│   ├── services/
│   │   └── archive-service.ts   # v2.2: 아카이브 관리
│   ├── tools/
│   │   ├── index.ts             # v2.0/v2.1/v2.2 도구 export
│   │   ├── context-search.ts    # v2.1: scope 파라미터, v2.2: context_tags 조인
│   │   ├── context-get.ts       # v2.2: 재귀 CTE 세션 체인
│   │   ├── context-warn.ts      # v2.1: 반복 실패 패턴 감지
│   │   ├── context-stats.ts     # v2.1: 세션 통계, v2.2: LEFT JOIN 최적화
│   │   ├── context-export.ts    # v2.1: 내보내기, v2.2: 배치 로딩
│   │   ├── context-recommend.ts # v2.1: 관련 세션 추천
│   │   └── context-archive.ts   # v2.2: 아카이브 도구
│   ├── sync/
│   │   └── json-sync.ts         # current.json 동기화
│   ├── utils/
│   │   └── truncate.ts          # 서버 사이드 요약 생성
│   └── types/
│       └── index.ts             # 타입 정의 (v2.1 확장)
├── docs/
│   └── v2.0-plan.md             # 이 문서
└── package.json
```
...
| v2.1 | 2025-12-27 | Phase 2 구현 완료: context_stats, context_export, context_recommend, 반복 실패 패턴 감지, 전역 검색 (global.db) |
| v2.2 | 2025-12-27 | Phase 3 구현 완료: 스키마 최적화 (context_tags, contexts_archive, 복합 인덱스), 쿼리 최적화 (태그 검색, N+1 수정, 재귀 CTE), context_archive 도구 |
